...
const formatDate = (timestamp: number) => { // format date 2:20 PM - 3/11/2024
...
const Message = ({ sender, text, timestamp }: MessageProps) => (  // show text, differentiate from user and assistant
  <div
    className={`message ${
...
const ChatInterface = () => {
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => { // send message on enter
  if (e.key === 'Enter') {
    e.preventDefault();
    sendMessage()
  }
  ...
  useEffect(() => { // scroll to bottom when new message is added
  ...
  return (
    <div className="h-96 w-96 bg-white  border border-blue-400 rounded-md flex flex-col justify-between p-">
      <div className=" overflow-y-auto"> {/* show messages custom no-scrollbar class to remove automatic scrollbar from overflow-y-auto*/}
